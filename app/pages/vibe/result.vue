...
const densityLabels: Record<string, { text: string; class: string }> = {
  ...
  red: { text: "🔥 Популярное", class: "bg-orange-500/15 text-orange-700" },
};
  ...
  <div class="min-h-screen bg-white border border-accent/40 flex flex-col items-center justify-start relative overflow-hidden px-5 pt-6 pb-16">
    ...
    <div class="relative z-10 w-full" :style="{ maxWidth: showRecs ? '1140px' : '36rem' }">
      <div class="text-center mb-8">
        <NuxtLink to="/start" class="font-body text-base text-primary-light no-underline mb-6 inline-flex items-center gap-1.5 hover:text-primary transition-colors">
          <ArrowLeft class="w-3.5 h-3.5" />
          Назад
        </NuxtLink>

        <div class="flex flex-col items-center gap-3 mt-4">
          <span class="inline-flex items-center gap-1.5 font-body font-bold text-xs tracking-[0.14em] uppercase text-accent-dark bg-accent/10 px-4 py-1.5 rounded-full">
            <Sparkles class="w-3.5 h-3.5" />
            Ваш вайб-паспорт
          </span>
          <h1 class="font-heading text-primary leading-[1.25] text-heading-page">
            {{ title }}
          </h1>
        </div>
      </div>
      ...
      <div class="bg-white/50 border border-accent/40 rounded-3xl p-8 md:p-10 mb-6 relative">
        <div v-if="vibeStore.profile?.vector_id" class="absolute top-4 right-5 sm:top-6 sm:right-6 flex items-center justify-end gap-1.5 opacity-30 select-none pointer-events-none">
          ...
          <span class="font-mono text-2xs text-white uppercase tracking-[0.2em]">{{ vibeStore.profile.vector_id.split('-').slice(0,2).join('-') }}</span>
        </div>
        ...
        <div class="bg-white/60 border border-accent/40 rounded-2xl p-5 mb-6">
          <p class="font-body text-base text-primary leading-relaxed italic">
            «{{ summary }}»
          </p>
        </div>
        ...
        <div v-if="vibeStore.profile?.transcription" class="bg-primary/5 rounded-2xl p-5 mb-6 border border-primary/10">
          <div class="flex items-center gap-1.5 mb-2 opacity-60">
            ...
            <span class="font-body text-xs font-bold text-primary uppercase tracking-wider">Распознанный запрос</span>
          </div>
          <p class="font-body text-base text-primary leading-relaxed">
        ...
        <div class="flex flex-wrap gap-2">
          <span
            v-for="tag in tags"
            :key="tag"
            class="font-body font-bold text-sm text-accent-dark bg-accent/10 px-3 py-1.5 rounded-full"
          >
            #{{ tag }}
          </span>
        </div>
      ...
      <p v-if="vibeStore.profile?.processing_time_ms" class="text-center font-body text-sm text-primary-light opacity-50 mb-4">
      ...
      <div v-if="!showRecs" class="flex flex-col gap-3 mb-6">
        <button
          class="flex items-center justify-center gap-2 font-body font-bold text-md text-white bg-accent rounded-2xl px-6 py-3.5 border-none cursor-pointer transition-all duration-200 hover:bg-accent-dark hover:-translate-y-0.5 hover:shadow-lg disabled:opacity-50 disabled:cursor-not-allowed"
        ...
        <div class="flex flex-col sm:flex-row gap-3">
          ...
          <NuxtLink
            to="/vibe/swipe"
            class="flex items-center justify-center font-body font-bold text-base text-primary bg-transparent border-2 border-primary/20 rounded-2xl px-6 py-3.5 no-underline transition-all duration-200 hover:border-accent hover:bg-accent hover:text-white"
          >
            Уточнить свайпами
          </NuxtLink>
        </div>
      ...
      <div v-if="showRecs" class="mt-8">
        <div class="flex items-center justify-between mb-5">
          <h2 class="font-body font-bold text-primary text-2xl">
            Top-{{ vibeStore.recommendations.length }} для вас
          </h2>
          <span v-if="vibeStore.isCurated" class="font-body text-sm text-primary-light bg-primary/8 rounded-full px-3 py-1">
            демо-подборка
          </span>
        </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-8">
          <NuxtLink
            ...
            class="group flex flex-col bg-white/50 border border-accent/40 rounded-[24px] overflow-hidden no-underline transition-all duration-250 hover:border-accent/40 hover:-translate-y-1 hover:shadow-[0_12px_40px_rgba(0,0,0,0.08)]"
          >
            ...
            <div class="flex flex-col gap-2.5 p-5">
              ...
              <div class="flex items-center gap-2 flex-wrap">
                <span class="font-body text-sm font-bold text-primary-light bg-primary/6 rounded-full px-2.5 py-0.5">
                  {{ categoryEmoji[rec.category] || '📍' }} {{ rec.category }}
                </span>
                <span
                  :class="densityLabels[rec.density_level]?.class"
                  class="font-body text-xs font-bold rounded-full px-2.5 py-0.5"
                >
                  {{ densityLabels[rec.density_level]?.text }}
                </span>
                <span v-if="rec.child_friendly" class="font-body text-xs font-bold text-blue-600 bg-blue-500/10 rounded-full px-2.5 py-0.5">
              ...
              <div class="flex items-start justify-between gap-2">
                <h3 class="font-body font-bold text-lg text-primary leading-snug">{{ rec.name }}</h3>
                <span class="shrink-0 flex items-center gap-1 font-body font-bold text-sm text-accent-dark bg-accent/15 rounded-lg px-2 py-1">
                  <Star class="w-3 h-3" />
              ...
              <p v-if="rec.description_short" class="font-body text-base text-primary-light leading-relaxed line-clamp-2">
                {{ rec.description_short }}
              </p>


              <p v-if="rec.reason_short" class="font-body text-sm text-accent-dark italic">
                {{ rec.reason_short }}
              </p>
              ...
              <div v-if="rec.tags_match?.length" class="flex flex-wrap gap-1.5">
                <span
                  ...
                  :key="mt"
                  class="font-body text-xs font-bold text-accent-dark bg-accent/10 rounded-full px-2 py-0.5 flex items-center gap-0.5"
                >
        ...
        <div class="flex flex-col sm:flex-row gap-3">
          <NuxtLink
            to="/trip/new"
            class="flex-1 flex items-center justify-center gap-2 font-body font-bold text-md text-white bg-accent rounded-2xl px-6 py-3.5 no-underline transition-all duration-200 hover:bg-accent-dark hover:-translate-y-0.5 hover:shadow-lg"
          >
            🗺 Собрать маршрут
