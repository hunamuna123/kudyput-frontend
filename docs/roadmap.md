...
1. Групповые поездки (invite URL генерируется при создании trip, но экран join пока не реализован).
2. Бронирование (кнопка «Забронировать» на странице локации, но без реальной логики).
3. ИИ-подбор маршрута (кнопка «ИИ, собери маршрут» на экране Route, но без backend-интеграции).

### Пока не реализовано

1. `/trip/:id/join` — экран присоединения к групповой поездке
2. Swipe по 3D-сценам (сейчас только текстовые карточки)
3. Booking flow (выбор дат, оплата)
4. Storytelling player для маршрута
5. Weather overlay на карте
6. Live rerouting
7. Hidden Gems / karma UX
8. B2G analytics views
9. Offline mode
10. Host dashboard (управление локациями хостом)
...
## Фаза F7.5. Техническое усиление после демо ✅ РЕАЛИЗОВАНО
...
2. **Hardcoded API endpoint → ✅ Исправлено:**
   - `useApiClient` использует `useRuntimeConfig().public.apiBase`.
   - Настраивается через `nuxt.config.ts` / environment variables.
   - Нет hardcoded IP-адресов.
...
1. ❌ **Join screen** `/trip/:id/join` — нет отдельного экрана присоединения.
2. ❌ **Members list** — нет UI для просмотра участников.
3. ❌ **Group preference summary** — нет сводки предпочтений группы.

---

## Phase F13. Host Onboarding Screens ✅ РЕАЛИЗОВАНО
...
### Статус: ✅ Реализовано (основной flow)
...
4. **Step 3 — Success:**
   - ✅ Confirmation screen.
   - ✅ CTA "Мои локации" (→ `/host/dashboard`), "Добавить ещё".
...
## Phase F15. Analytics/B2G/Full GDD ❌ НЕ РЕАЛИЗОВАНО
...
1. 🔶 Admin dashboard `/admin/index` — базовая страница (shell).
2. 🔶 Admin locations `/admin/locations` — управление локациями (базовый список).
...
- 🔶 demo-хаки заменены внутри без редизайна экранов (осталось: booking, group join, AI route).
...
| F7.5 | Техническое усиление | ✅ Реализовано |
...
| F13 | Host Onboarding | ✅ Реализовано |
| F14 | Booking/Story/Weather | ❌ Не реализовано |
| F15 | Analytics/B2G/Full GDD | ❌ Не реализовано |
