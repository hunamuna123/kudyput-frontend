...
### 🧠 Фича 1: Multi-Modal Vibe Profiling

**Цель**: ИИ понимает «вайб» туриста двумя способами, не задавая душных вопросов.
...
После завершения профилирования (голос + свайпы) пользователь видит:

- Свой «вайб-паспорт» (красивая визуализация)
- Top-5 подобранных локаций с 3D-превью
...
- Точки локаций — светящиеся маркеры (разный цвет по категории)
...
1. После профилирования юзер видит 3D-карту с подсвеченными точками-рекомендациями
2. Вращает карту, рассматривает
3. Наводит на точку — всплывает tooltip: название, одна фраза описания, рейтинг
4. Кликает — открывается карточка локации (сайдбар с 3D-превью через Splatting)
5. Может добавить несколько точек → система строит маршрут (неоновый сплайн)
6. Нажимает «Поехали» → маршрут сохраняется, доступно бронирование
...
journey
    ...
    section Осознание
      Видит рекламу / ссылку: 3: Tourist
      Заходит на deepkrai.ru: 4: Tourist
    section Профилирование
      Наговаривает голосовое: 5: Tourist, AI
      Свайпает 3D-сцены: 5: Tourist, AI
      Получает вайб-паспорт: 5: Tourist
    section Исследование
      Изучает 3D-карту с погодой: 5: Tourist
      Входит в 3D-экскурсию (Splatting): 5: Tourist
      Читает AI-описания локаций: 4: Tourist
    section Планирование
      Добавляет точки в маршрут: 4: Tourist
      AI строит оптимальный маршрут: 5: Tourist, AI
    ...
    section В пути
      ...
      Погода меняется — маршрут перестроен: 5: Tourist, AI
      Навигация без интернета: 4: Tourist
    section После
      Оставляет отзыв: 4: Tourist
      Карма растёт: 4: Tourist
      Открываются Hidden Gems: 5: Tourist
```
...
| `GET`  | `/api/v1/map/locations`             | location     | Локации на карте (с фильтрами, bbox)        |
...
| `POST` | `/api/v1/route/build`               | routing      | Построить маршрут                           |
